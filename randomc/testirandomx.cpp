...
int main() {
   // Define constants
   const int nsamp  =  8005000 * 16;   // Number of samples
   ...
   CRandomSFMT1 ran1(seed);               // Make instance of random number generator
   for (i = 0; (int)i < period; i++) x[i] = 0;  // Initialize

   printf("Test difference between IRandom and IRandomX...\n");

   // Sample loop with IRandom
   for (i = 0; (int)i < nsamp; i++) {
   ...
   for (i = 0; (int)i < period; i++) printf("%i  %9u\n", i, x[i]);

   // Repeat test with IRandomX
   ran1.RandomInit(seed);              // Reset generator with same seed
   for (i=0; (int)i < period; i++) x[i] = 0;  // Initialize again
